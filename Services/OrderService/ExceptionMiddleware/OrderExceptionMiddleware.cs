...
namespace OrderService.ExceptionMiddleware
{
    ...
    /// </summary>
    public class OrderExceptionMiddleware
    {
        private readonly RequestDelegate _next;
        private readonly ILogger<OrderExceptionMiddleware> _logger;
        private readonly IHostEnvironment _env;

        public OrderExceptionMiddleware(
            RequestDelegate next,
            ILogger<OrderExceptionMiddleware> logger,
        ...
        private async Task HandleExceptionAsync(HttpContext context, Exception ex)
        {
            ...
            switch (ex)
            {
                ...
                // Order-lifecycle and stock-reservation conflicts are expected,
                // business-level outcomes — not bugs — so they're logged as
                // warnings, not errors, and never trigger retries/alerts.
                case InvalidOrderStateException or StockUnavailableException:
                    var appEx1 = (AppException)ex;
                    statusCode = appEx1.StatusCode;
                    _logger.LogWarning(ex, "Order state/stock conflict. TraceId={TraceId}", traceId);
                    problem = new ProblemDetails
                    {
                        Status = statusCode,
                        Title = "Request conflicts with current order/stock state.",
